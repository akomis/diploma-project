...
class Device():
    ...
    def __init__(self, config, port):
        ...
        self.timeout = self.section.getint('Timeout', fallback=globals["timeout"])
        if (self.timeout < 100):
            self.timeout = 100
...
class Dobot(Device):
    ...
    def __prominit(self):
        ...
        if self.section.getboolean('deviceversion', fallback=options["deviceversion"]):
...
class Jevois(Device):
    ...
    def __prominit(self):
        if self.section.getboolean("objectidentified", fallback=self.options["objectidentified"]):
